...
def user_language(db_access: Database, message: Message|CallbackQuery) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
...
def give_name(db_access: Database, message: Message|CallbackQuery) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def get_user_name(db_access: Database, user_id: int) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def get_group_name(db_access: Database, message: Message|CallbackQuery, group_id: int) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    cursor.execute(f"SELECT group_name FROM Groups WHERE group_id = {group_id}")
    group_name = cursor.fetchone()[0]

    if group_name == user_id:
        db.close()
        return LEXICON['my_own_group'][user_language(db_access, message)]
    else:
        db.close()
        return group_name
...
def get_gift_name(db_access: Database, gift_id: int) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def get_giver_id(db_access: Database, gift_id: int) -> int:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def users_in_groups(db_access: Database, message: Message|CallbackQuery, not_all=True) -> dict:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    for group_id in all_accessible_gifts(db_access, message):
        if group_id not in all_users:
            all_users[group_id] = []
        for user_id in all_accessible_gifts(db_access, message)[group_id]:
    ...
    if not_all:
        ...
        all_users.pop(cursor.fetchone()[0])
...
def users_gifts_in_group(db_access: Database, message: Message | CallbackQuery) -> dict:
    ...
    for user_id in all_accessible_gifts(db_access, message)[group_id]:
        if user_id not in all_gifts:
            all_gifts[user_id] = []
        for gift in all_accessible_gifts(db_access, message)[group_id][user_id]:
...
def user_gifts_in_group(db_access: Database, message: Message | CallbackQuery) -> list:
    ...
    for gift_id in all_accessible_gifts(db_access, message)[group_id][user_id]:
...
def all_users_gifts(db_access: Database, message: Message|CallbackQuery) -> dict:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def feedback_list(db_access: Database, ) -> list:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def issue_list(db_access: Database, ) -> list:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def all_my_own_gifts(db_access: Database, message: Message|CallbackQuery) -> str:
    user_id = int(message.from_user.id)
    all_gifts = all_accessible_gifts(db_access, message)

    gift_list = ''

    for group_id in all_gifts.keys():
        gift_list += f'<b>{get_group_name(db_access, message, group_id)}</b>:\n'
        for gift_id in all_gifts[group_id][user_id]:
            gift_list += '    ' + get_gift_name(db_access, gift_id) + '\t\n'
        gift_list += '\t\n'

    if gift_list == f"<b>{LEXICON['my_own_group'][user_language(db_access, message)]}</b>:\n\t\n":
        gift_list = LEXICON['no_gifts'][user_language(db_access, message)]

    return gift_list


##### ВОЗВРАЩАЕТ ВСЕ ГРУППЫ ПОЛЬЗОВАТЕЛЯ С ИХ СОСТАВАМИ #####
def all_my_groups(db_access: Database, message: Message | CallbackQuery) -> str:
    all_users = users_in_groups(db_access, message)
    user_list = ''

    for group_id in all_users.keys():
        user_list += f'<b>{get_group_name(db_access, message, group_id)}</b>:\n'
        for user_id in all_users[group_id]:
            user_list += f'    {get_user_name(db_access, user_id)}\n'
        user_list += '\n'
...
def all_users_in_group(db_access: Database, message: Message | CallbackQuery) -> str:
    all_gifts = users_gifts_in_group(db_access, message)
    group_id = int(message.data)
    gift_list = f"<u><b>{get_group_name(db_access, message, group_id).upper()}</b></u>\n\n"

    for user_id in all_gifts.keys():
        gift_list += f'<b>{get_user_name(db_access, user_id)}</b>:\n'

        for gift_id in all_gifts[user_id]:
            gift_list += f'    {get_gift_name(db_access, gift_id)}\n'
        gift_list += '\n'
...
def all_gifts_by_user_in_group(db_access: Database, message: Message|CallbackQuery, status=False) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    user_id = int(message.data[message.data.index('_') + 1:])
    all_gifts = user_gifts_in_group(db_access, message)
    gifts = f"<u><b>{get_user_name(db_access, user_id).upper()}</b></u>" + '\n\n'
    ...
    if status:
        for gift in all_gifts:
            ...
            gifts += f"{is_free}{get_gift_name(db_access, gift)}\n"
    else:
        for gift in all_gifts:
            gifts += f"{get_gift_name(db_access, gift)}\n"
...
def what_to_do_with_gift(db_access: Database, callback: CallbackQuery) -> str:
    gift_id = int(callback.data[callback.data.rfind('_') + 1:])
    giver_id = get_giver_id(db_access, gift_id)
    gift_name = get_gift_name(db_access, gift_id)

    if callback.from_user.id == giver_id:
        return f"<b>{gift_name.upper()}</b>\n\n" + LEXICON['free_up_gift'][user_language(db_access, callback)]
    elif giver_id == 0:
        return f"<b>{gift_name.upper()}</b>\n\n" + LEXICON['take_gift'][user_language(db_access, callback)]
    else:
        return f"<b>{gift_name.upper()}</b>\n\n" + LEXICON['taken_gift'][user_language(db_access, callback)]


##### ВОЗВРАЩАЕТ СПИСОК ОТЗЫВОВ #####
def all_feedback(db_access: Database, message: Message | CallbackQuery) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    if len(cursor.fetchall()) > 0:
        ...
        for row in cursor.fetchall():
            if type(row[2]) == int:
                name = get_user_name(db_access, row[2])
            else:
                name = 'None'
            feedback += str(row[0]) + '. ' + name + ': ' + row[1] + '\n\n'
    else:
        feedback = LEXICON_ADMIN['no_feedback'][user_language(db_access, message)]
...
def all_issues(db_access: Database, message: Message | CallbackQuery) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    if len(issues_data) > 0:
        for row in issues_data:
            if row[2]:
                ...
                issues += LEXICON_ADMIN['issue'][user_language(db_access, message)]
            issues += str(row[0]) + '. ' + row[1] + '\n\n'
    else:
        issues = LEXICON_ADMIN['no_issue'][user_language(db_access, message)]
...
def user_in_data(db_access: Database, message: Message|CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def new_user(db_access: Database, message: Message|CallbackQuery, user_name: str) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def is_user_has_gift(db_access: Database, message: Message|CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def is_gift_in_group(db_access: Database, group_id: int, gift_id: int) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def create_new_gift(db_access: Database, message: Message|CallbackQuery) -> int:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def add_new_gift_in_group(db_access: Database, message: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    if get_group_name(db_access, message, chosen_group_id) == LEXICON['my_own_group'][user_language(db_access, message)]:
        cursor.execute(
            f"SELECT group_id FROM Group_User "
            f"WHERE user_id = {user_id}"
        )
        accessible_group_ids = [row[0] for row in cursor.fetchall()]
...
def take_gift(db_access: Database, callback: CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def free_up_gift(db_access: Database, callback: CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def delete_gift(db_access: Database, message: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    if group_id == int(cursor.fetchone()[0]):
        ...
        cursor.execute(f"SELECT * FROM Group_Gift WHERE gift_id = {gift_id}")
        if len(cursor.fetchall()) == 0:
            cursor.execute(f"DELETE FROM Gifts WHERE gift_id = {gift_id}")
            db.commit()
...
def is_user_has_group(db_access: Database, message: Message|CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def new_group(db_access: Database, message: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    while True:
        ...
        cursor.execute('SELECT password FROM Groups')
        if not(group_password in [i[0] for i in cursor.fetchall()]):
            break
    ...
    cursor.execute(f"SELECT group_id FROM Groups WHERE password = '{str(owner_id)}'")
    my_own_group_id = cursor.fetchone()[0]

    cursor.execute(f"SELECT group_id FROM Groups WHERE password = '{group_password}'")
    group_id = cursor.fetchone()[0]
...
def is_user_in_group(db_access: Database, message: Message) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    user_id = int(message.from_user.id)
    cursor.execute(f"SELECT group_id FROM Groups WHERE password = '{password}'")
    group_id = cursor.fetchone()[0]
...
def is_user_has_same_group(db_access: Database, message: Message|CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    password = str(message.text)
    user_id = int(message.from_user.id)
    cursor.execute(f"SELECT group_name FROM Groups WHERE password = '{password}'")
    group_name = cursor.fetchone()[0]

    cursor.execute(f"SELECT group_name "
                   f"FROM Groups "
                   f"INNER JOIN Group_User "
                   f"ON Groups.group_id = Group_User.group_id "
                   f"WHERE user_id = {user_id}")
...
def get_password(db_access: Database, message: Message | CallbackQuery) -> str:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    cursor.execute(f"SELECT password FROM Groups WHERE group_id = {group_id}")
...
def is_password_correct(db_access: Database, message: Message | CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    password = str(message.text)
    cursor.execute("SELECT password FROM Groups")
    if password in [row[0] for row in cursor.fetchall()]:
        db.close()
        return True
    else:
        db.close()
...
def add_user_in_group(db_access: Database, message: Message) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    for_everyone_gift_ids = [row[0] for row in cursor.fetchall()]
...
def user_is_owner(db_access: Database, callback: CallbackQuery) -> bool:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def kill_group(db_access: Database, callback: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    gifts_in_group = [row[0] for row in cursor.fetchall()]

    cursor.execute(f"DELETE FROM Groups WHERE group_id = {group_id}")
    db.commit()

    for gift_id in gifts_in_group:
        cursor.execute(f"SELECT group_id FROM Group_gift WHERE gift_id = {gift_id}")
        if len(cursor.fetchall()) == 0:
            cursor.execute(f"DELETE FROM Gifts WHERE gift_id = {gift_id}")
            db.commit()
...
def all_accessible_gifts(db_access: Database, message: Message|CallbackQuery) -> dict:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    for group_id in accessible_group_ids:
        ...
        users_in_group = [row[0] for row in cursor.fetchall()]
        ...
        for user_id in users_in_group:
            ...
            gifts_from_user = [row[0] for row in cursor.fetchall()]
...
def new_feedback(db_access: Database, message: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def kill_feedback(db_access: Database, callback: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def new_issue(db_access: Database, callback: Message | CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
    ...
    cursor.execute(f"SELECT feedback_text "
                   f"FROM Feedback "
...
def solve_issue(db_access: Database, callback: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def not_solve_issue(db_access: Database, callback: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
...
def kill_issue(db_access: Database, callback: Message|CallbackQuery) -> None:
    db = psycopg2.connect(
        dbname=db_access.db_name,
        user=db_access.db_user,
        password=db_access.db_password,
